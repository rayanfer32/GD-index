//Overrides (comment if not needed) 
var main_color = "pink";var accent_color = "light-blue";var dark = true;
...
if(dark){document.write('<style>* {box-sizing: border-box}body{color:rgba(255,255,255,.87);background-color:#333232}.mdui-theme-primary-'+main_color+' </style>');}
...
function append_files_to_list(path, files) {
  ...
  for (i in files) {
    ...
    if (item['mimeType'] == 'application/vnd.google-apps.folder') {
      ...
      html += `<li class="mdui-row file mdui-ripple" target="_blank"><a gd-type="${item.mimeType}" href="${p}" class="${c}"> 
	          <div class="mdui-col-xs-6 mdui-col-sm-4 mdui-text-truncate">${item.name}</div></a>
            <div class="mdui-col-xs-4 mdui-col-sm-3 mdui-text-truncate">
            <img src="https://drive.google.com/thumbnail?id=${item['id']}&sz=w120" onerror="this.onerror=null;this.src='https://via.placeholder.com/120x75?text=NOT%20PUBLIC';"></img>
            </div>
            <div class="mdui-col-xs-2 mdui-col-sm-1" >
              <a href="https://drive.google.com/file/d/${item['id']}/preview"><i class="mdui-icon material-icons">play_circle_outline</i>DRIVE</a>
            </div>   
	          <div class="mdui-col-xs-2 mdui-col-sm-1 ">${item['size']}</div>
...
function get_file(path, file, callback) {
  console.log(path, file, callback);
